...
export const StatCard: React.FC<StatCardProps> = ({ title, value, icon, trend }) => {
  ...
  let fontSizeClass = 'text-2xl sm:text-3xl md:text-4xl';
    ...
    <div className="group relative bg-gradient-to-br from-slate-800/90 via-slate-800/80 to-slate-900/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl p-6 hover:shadow-primary-500/20 hover:shadow-2xl hover:border-primary-500/50 hover:-translate-y-1 transition-all duration-500 overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          {icon && (
            <div className="flex-shrink-0 p-2.5 bg-gradient-to-br from-primary-500/30 to-primary-600/20 rounded-lg group-hover:from-primary-500/40 group-hover:to-primary-600/30 group-hover:scale-110 transition-all duration-300 shadow-lg">
        ...
        <div className="min-w-0 w-full">
          <p className={`${fontSizeClass} font-extrabold text-white mb-2 bg-gradient-to-r from-white via-gray-100 to-gray-300 bg-clip-text text-transparent drop-shadow-lg break-words leading-tight`}>
            {formattedValue}
          </p>
          {trend && (
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              <span className={`text-xs sm:text-sm font-bold px-2 py-1 rounded-md ${
                trend.isPositive 
