...
async def update_user_role(
    ...
    app_logger.info(f"User {user_id} role changed from {old_role} to {request.role} by admin {admin.id}")
    
    return {
        "message": f"User role updated from {old_role} to {request.role} successfully",
        "user": user
    }
